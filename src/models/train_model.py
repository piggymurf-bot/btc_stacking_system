...
def run_training_pipeline():
    ...
    X_holdout = X_holdout.replace([np.inf, -np.inf], np.nan).ffill().bfill()
    
    #Extract Close price for holdout dates
    close_df = pd.DataFrame()
    close_df['close'] = df.loc[X_holdout.index, 'Close']
    ...
    stacker.fit_stack(X_train, y_train)
    ...
    # Generate Holdout Predictions for Phase 4 (Backtesting)
    holdout_probs = stacker.predict_proba_stack(X_holdout)[:, 1]
